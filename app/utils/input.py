...
class Key:
    ...
    def simulate(hex_key_code, key_down=True):
        ...
        if isinstance(hex_key_code, str):
            if hex_key_code.startswith("0x"):
                hex_key_code = hex_key_code[2:]
            key_code_int = int(hex_key_code, 16)
        else:
            key_code_int = hex_key_code

        extra = ctypes.c_ulong(0)
        ii_ = Input_I()
        flags = KEYEVENTF_KEYDOWN if key_down else KEYEVENTF_KEYDOWN | KEYEVENTF_KEYUP
        ii_.ki = KeyBdInput(0, key_code_int, flags, 0, ctypes.pointer(extra))
