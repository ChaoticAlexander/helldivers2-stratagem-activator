...
def simulate_key(hex_key_code, key_down=True):
...
def press_key(hex_key_code, delay=0.02):
    """Simulate pressing a key with an optional delay before releasing."""
    simulate_key(hex_key_code, key_down=True)
    time.sleep(delay)
    simulate_key(hex_key_code, key_down=False)
